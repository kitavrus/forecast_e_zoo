// Package config грузит настройки сервиса data-marts (Модуль 3) из переменных окружения.
package martsconfig
...
// Config — корневой конфиг сервиса data-marts.
type Config struct {
	// HTTP
	HTTPAddr string `envconfig:"MARTS_HTTP_ADDR" default:":8082"`
...
func Load() (*Config, error) {
	...
	if err := envconfig.Process("", &cfg); err != nil {
		return nil, fmt.Errorf("marts config: envconfig: %w", err)
	}
	return &cfg, nil
}
