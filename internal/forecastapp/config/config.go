// Package config грузит настройки сервиса forecast (Модуль 5) из переменных окружения.
package forecastappconfig
...
// Config — корневой конфиг сервиса forecast.
type Config struct {
	// HTTP
	HTTPAddr string `envconfig:"FORECAST_HTTP_ADDR" default:":8084"`
	...
	// Scheduler
	CronSchedule string `envconfig:"FORECAST_CRON_SCHEDULE" default:"0 5 * * *"`
	CronTZ       string `envconfig:"FORECAST_CRON_TZ" default:"Europe/Kyiv"`
	HorizonDays  int    `envconfig:"FORECAST_HORIZON_DAYS" default:"60"`
...
func Load() (*Config, error) {
	...
	if err := envconfig.Process("", &cfg); err != nil {
		return nil, fmt.Errorf("forecast config: envconfig: %w", err)
	}
	return &cfg, nil
}
