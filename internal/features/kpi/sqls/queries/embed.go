// Package queries содержит embed.FS со SQL-запросами фичи kpi.
//
...
func getOrError(name string) (string, error) {
	if strings.ContainsAny(name, "/\\") {
		return "", fmt.Errorf("kpi queries.Get: bad name %q", name)
	}
	raw, err := fs.ReadFile(name + ".sql")
	if err != nil {
		return "", fmt.Errorf("kpi queries.Get: %s.sql not found: %w", name, err)
	}
